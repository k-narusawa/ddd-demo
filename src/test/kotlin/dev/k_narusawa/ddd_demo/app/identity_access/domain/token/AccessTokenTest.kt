...
@DisplayName("ドメイン_値オブジェクト_アクセストークン")
class AccessTokenTest {
  ...
  inner class Instantiation {
    ...
    fun `can instantiate with fromJwt method`() {
      val userId = UserId.new()
      val token = AccessToken.generate(
      ...
      val sut = AccessToken.fromJwt(token.get(), SECRET)
    ...
    fun `fromJwt with invalid jwt throws exception`() {
      assertThrows(TokenUnauthorized::class.java) {
        AccessToken.fromJwt("invalid_jwt", SECRET)
      }
    }
  }
  ...
  inner class Equality {
    ...
    fun `instances with same value are equal`() {
      ...
      val sut1 = AccessToken.fromJwt(token.get(), SECRET)
      val sut2 = AccessToken.fromJwt(token.get(), SECRET)
    ...
    fun `instances with different values are not equal`() {
      val userId = UserId.new()
      val token1 = AccessToken.generate(
        tokenId = TokenId.generate(),
        userId = userId,
        secret = SECRET,
        expiresIn = EXPIRES_IN
      )
      val token2 = AccessToken.generate(
        tokenId = TokenId.generate(),
        userId = userId,
        secret = SECRET,
        expiresIn = EXPIRES_IN
      ...
      val sut1 = AccessToken.fromJwt(token1.get(), SECRET)
      val sut2 = AccessToken.fromJwt(token2.get(), SECRET)
